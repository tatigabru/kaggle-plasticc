...
training_set_selected = pd.read_csv('../Features_New_Aug_Calc/augmented_basic_features_v2.csv')
...
other_basic_features = pd.read_csv('../Features/train_set_cesium_full_features.csv')
print('train_set_cesium_full_features loaded')
other_basic_features['object_id'] = 100 * other_basic_features['object_id'] + 10
...
used_columns = ['det_mjd_diff', 'hostgal_photoz', 'det_magn_min', 'exp_ratio_fitting_loss', 'det_magn_mean', 'flux_by_flux_ratio_sq_skew', 'fm_0', '__median_absolute_deviation___5_', 'detected_mean', 'det_magn_std', 'gauss_s_2', 'gauss_err_5', 'period', 'cvec1', 'cvec5', '__median_absolute_deviation___4_', 'tau_fall', 'fg_i', 'g_r', 'gauss_s_3', 'my_6_certain_width', 'CAR_tau', 'magn_fm_4', 'flux_err_min', 'det_magn_mean_2', 'fg_z', 'flux_diff2', 'gauss_s_4', 'my_6_est_width', 'det_magn_sum_2', 'cvec0', '__median_absolute_deviation___2_', 'gauss_err_2', 'm_15_5', 'magn_fm_5', 'cvec3', 'fi_z', '__skew___2_', 'gauss_err_1', 'fr_z', 'det_magn_mean_5', 'fm_5', 'gauss_err_4', 'fm_2', 'i_z', 'cvec2', 'm_15_3', 'gauss_s_1', 'supernova1_s2_3', 'det_magn_sum_5', '__qso_log_chi2_qsonu___0_', 'm_15_1', 'det_magn_sum_3', '__median_absolute_deviation___3_', 'det_magn_min_2', 'supernova1_s2_2', 'det_magn_sum_4', 'r_i', '__qso_log_chi2_qsonu___2_', 'magn_fit_5', '__skew___1_', 'time_score', 'gauss_s_5', 'det_magn_mean_0', 'm_15_2', 'det_magn_mean_4', '__qso_log_chi2_qsonu___5_', 'magn_fit_4', 'det_magn_min_5', 'magn_fm_1', 'tau_rise', 'fu_r', 'm_15_4', 'det_magn_sum_0', 'fm_1', 'gauss_s_0', 'magn_fit_1', 'det_magn_min_4', 'magn_fit_2', '__freq_varrat___4_', 'det_magn_min_0', 'g_i', '__stetson_k___1_', 'cvec4', 'r_z', 'fm_3', 'fr_i', 'det_magn_mean_3', 'g_z', '__freq_varrat___1_', 'det_magn_min_1', 'flux_skew', 'gauss_err_3', '__qso_log_chi2_qsonu___1_', 'det_magn_mean_1', 'CAR_sigma', 'magn_fm_0', 'fg_r', 'u_r', '__skew___3_']
used_columns.append('object_id')
used_columns.append('target')
used_columns.sort()
...
full_train = pd.merge(left=full_train, right=training_set_car, on='object_id', how='inner')
full_train = pd.merge(left=full_train, right=training_set_fits, on='object_id', how='inner')
full_train = pd.merge(left=full_train, right=training_set_cesium, on='object_id', how='inner')
...
assert 'target' in set(full_train.columns)
print('len after merge', len(full_train))

filter = (training_set_metadata['aug_fold'] == 10)
...
full_train = pd.merge(left=full_train, right=other_basic_features, on='object_id', how='inner', suffixes=('', 'extra_'))
print('after merge original features', len(full_train))
...
for fold_, (trn_, val_) in enumerate(augmented_split(split_df, folds)):
    ...
    print(len(training_set_metadata[(training_set_metadata['object_id'].isin(valid_objects)) & (training_set_metadata['ddf'] == 1)]))
...
def do_train(full_train, train_columns, only_one_fold=False, show_confusion_matrix=False, save_importances=False):
    ...
    if len(clfs) > 1:
        ...
        if show_confusion_matrix:
            ...
            plot_confusion_matrix(cnf_matrix, classes=columns_14_short, normalize=True, title='Confusion matrix')
...
best_loss, clfs = do_train(full_train, best_columns, only_one_fold=only_one_fold, show_confusion_matrix=True,
...
for i_c, dfs in enumerate(zip(
        ...
        pd.read_csv( '../Features/test_cesium_filtered_for_paper.csv', iterator=True, chunksize=chunks),
    ...
    preds_99 = ((((((mymedian) + (((mymean) / 2.0)))/2.0)) + (((((1.0) - (((mymax) * (((mymax) * (mymax))))))) / 2.0)))/2.0)
...
columns_galactic = ['class_6', 'class_16', 'class_53', 'class_65', 'class_92',]
columns_intergalactic = ['class_15', 'class_42', 'class_52', 'class_62', 'class_64',
              'class_67', 'class_88', 'class_90', 'class_95']
