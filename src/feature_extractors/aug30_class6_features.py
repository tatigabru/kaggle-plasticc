...
def calc_peak_width(df_lights, passband):
    ...
    while ind < fl:
        ...
        if start == 0:
            ...
            peak_start = find_root(p['mjd'].values[start - 1], fv[start - 1] - thr, p['mjd'].values[start], fv[start] - thr)
        if end == fl:
...
# n_peaks, est_peak_start, est_peak_end, certain_peak_start, certain_peak_end
...
def calc_features_for_df(df_lights):
    ...
    return pd.DataFrame(data=features_for_all_objects,
