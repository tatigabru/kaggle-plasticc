...
def fit_function(x, fm, tau_rise, tau_fall, t0, f0):
    """
    Double exponent fitting function
    """
...
def m15_m10(fit, meta, object_id, mean, std, passband):
    ...
    for t in t1:
        if (
            np.abs(fit_function(t, fm, tau_rise, tau_fall, t0, f0) - fmax / 2)
        ) < epsilon:
            t_fall50 = t
        if (
            np.abs(fit_function(t, fm, tau_rise, tau_fall, t0, f0) - fmax / 5)
        ) < epsilon:
            t_fall20 = t
        if (
            np.abs(fit_function(t, fm, tau_rise, tau_fall, t0, f0) - 0.8 * fmax)
        ) < epsilon:
            t_fall80 = t
...
def m_features(train, fit, objects, passband=2):
    ...
    for obj in objects:
        ...
        mean, std = (
            train[train["object_id"] == obj]["flux"].mean(),
            train[train["object_id"] == obj]["flux"].std(),
        )

        object_ids.append(obj)

        fm, Mfm, Mmax, m15, m10, t50, t20, t80 = m15_m10(
            fit, meta, obj, mean, std, passband
        )
...
def main():
    ...
    m, sd = (
        train[train["object_id"] == object_id]["flux"].mean(),
        train[train["object_id"] == object_id]["flux"].std(),
    )

    fm, Mfm, Mmax, m15, m10, t_50, t_20, t_80 = m15_m10(
        fit, meta, object_id, m, sd, passband=2
    )
    ...
    for pb in range(6):
        ...
        pd.DataFrame.from_dict(m_feat).to_csv(
            "train_fit_features_%d.csv" % pb, index=False
        )
