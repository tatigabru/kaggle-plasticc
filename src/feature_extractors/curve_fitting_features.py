...
class GaussianCurve(nn.Module):
    def __init__(self, batch_size=20, n_bands=6):
        ...
        self.params = nn.ParameterDict({
            'f0':
                nn.Parameter(torch.zeros(batch_size, dtype=torch.float)),
            'lambda':
                nn.Parameter(torch.ones(batch_size, dtype=torch.float)),
            't0':
                nn.Parameter(torch.ones(batch_size, dtype=torch.float)),
            'fm':
                nn.Parameter(torch.ones(batch_size, n_bands, dtype=torch.float))
        })
    ...
    def forward(self, times, masks=None):
        ...
        z = torch.exp(-z)
        y = self.params['fm'].unsqueeze(1) * z.unsqueeze(2) + self.params['f0'].reshape(-1, 1, 1)
    ...
    def predict(self, times, n):
        ...
        z = torch.exp(-z)
        y = self.params['fm'][n, :].reshape(1, -1) * z.reshape(-1, 1) + self.params['f0'][n]
        return y
...
class ExpRatioCurve(nn.Module):
    ...
    def forward(self, times, masks=None):
        t_f = -(
                times.unsqueeze(2) - self.params['t0'].unsqueeze(1)) * torch.clamp(
        ...
        y = self.params['fm'].unsqueeze(1) * z + self.params['f0'].reshape(
            -1, 1, 1)
        if masks is not None:
            y = y[masks]
        return y
    ...
    def predict(self, times, n):
        t_f = -(times.unsqueeze(1) -
                self.params['t0'][n].unsqueeze(0)) * torch.clamp(
            self.params['lambda_fall'][n], min=0)
        t_r = -(times.unsqueeze(1) -
                self.params['t0'][n].unsqueeze(0)) * torch.clamp(
...
def penalty(t0, k=1):
    return torch.mean(k * torch.sum((t0 - torch.mean(t0, dim=1, keepdim=True)) ** 2, dim=1))


def batch_fit(chunk, n_iters_gaussian=5000, n_iters_exp=10000, sample_plot=False, augmentation_set=False):
    # normalise mjd
    ...
    for i in range(n_iters_gaussian):
        ...
        nn.utils.clip_grad_norm_(curve.parameters(), 1)  # clip gards to ensure stability
        optimiser.step()
        #     scheduler.step()
        if i % 1000 == 0:
    ...
    curve2 = ExpRatioCurve(batch_size=selected[group_col].unique().shape[0], refine=True).cuda()
    ...
    for i in range(n_iters_exp):
        ...
        for k, param in curve2.params.items():
            if torch.sum(torch.isnan(param.grad)) > 0:
                param.grad.data[torch.isnan(param.grad)] = 0
                warnings.warn(f'NaN encountered in grad of {k}', RuntimeWarning)
        nn.utils.clip_grad_norm_(curve2.parameters(), 1)  # clip gards to ensure stability
        optimiser.step()
        scheduler.step()
        if i % 1000 == 0:
            print(f'loss={loss.detach().cpu().numpy():.5f}, penalty={p.detach().cpu().numpy():5f}')
    ...
    if sample_plot:
        for n in range(10):
            ...
            y = curve2.predict(torch.tensor(t_range, dtype=torch.float).cuda(), n).detach().cpu().numpy()
            for band in range(6):
                ax.plot(t_range, y[:, band], c=colors[band], alpha=0.5)
            ax.set_title(f'object {obj[group_col].iloc[0]}, '
    ...
    per_obj_loss = pd.Series(raw_loss.detach().cpu().numpy()).groupby(selected[group_col]).mean()
    if augmentation_set:
        params_df = pd.DataFrame({
            'augmentation_id': selected['augmentation_id'].unique(),  # use only for augmentation
            'exp_ratio_fitting_loss': per_obj_loss,
            'tau_rise': 1 / curve2.params['lambda_rise'].data.detach().cpu().numpy(),
            'tau_fall': 1 / curve2.params['lambda_fall'].data.detach().cpu().numpy(),
            'f0': curve2.params['f0'].data.detach().cpu().numpy()
        })
    else:
        params_df = pd.DataFrame({
            'object_id': selected['object_id'].unique(),  # use only for non-augmentation
            'exp_ratio_fitting_loss': per_obj_loss,
            'tau_rise': 1 / curve2.params['lambda_rise'].data.detach().cpu().numpy(),
            'tau_fall': 1 / curve2.params['lambda_fall'].data.detach().cpu().numpy(),
            'f0': curve2.params['f0'].data.detach().cpu().numpy()
...
def main():
    ...
    for i, part in tqdm(enumerate(test_series.partitions), total=test_series.npartitions):
        chunk = part.compute().reset_index()
        params_df = batch_fit(chunk, n_iters_gaussian=N_gaussian, n_iters_exp=N_exp, sample_plot=True)
        break
        params_df.to_csv(path.join(OUTPUT_DIR, f'test_exp_ratio_fitted_{i}.csv'), index=False)

    """
    Fitting aug 30x set
    """
    print('*' * 20 + '\nReading data...')
    train_series = pd.read_csv('data/gp_augmented/gp_augmented_ddf_to_nonddf_class_52.csv')
