...
def feets_features_band(df, feature_list, object_id, passband=0, passband2=2):
    ...
    """
    lc_b = df[(df.object_id == object_id) & (df.passband == passband)][
        ["mjd", "flux", "flux_err"]
    ]
    lc_r = df[(df.object_id == object_id) & (df.passband == passband2)][
        ["mjd", "flux", "flux_err"]
    ]
    ...
    atime, amag, amag2, aerror, aerror2 = feets.preprocess.align(
        ...
        lc_b.flux.values,
        lc_r.flux.values,
        lc_b.flux_err.values,
        lc_r.flux_err.values,
    )
    # light curve
    lc = [
        lc_b.mjd.values,
...
def worker(part):
    ...
    for object_id in objects[1:]:
        ...
        values = np.column_stack(
            [values, feat_values_]
        )  # numpy array of features values
    ...
    print(feets_df.head())
    feets_df.to_csv(
        FEAT_DIR + "feets_selected_augmented_train" + str(part) + ".csv",
        header=True,
        index=False,
    )
