...
def _car_like(parameters, t, x, error_vars):
    ...
    for i in range(1, N):
        ...
        x_hat[i] = A[i - 1] * x_hat[i - 1] + (
            A[i - 1] * Omega[i - 1] / (Omega[i - 1] + error_vars[i - 1])
        ) * (x_ast[i - 1] - x_hat[i - 1])
...
def calculate_car(time, magnitude, error, minimize_method="nelder-mead"):
    ...
    res = minimize(
        _car_like, x0, args=(time, magnitude, e), method=minimize_method, bounds=bounds
    )
