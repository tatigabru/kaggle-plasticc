...
def calc_features_for_passband(df_lights):
    ...
    s = np.log(1 + df_lights["flux_err"].values / np.clip(df_lights["flux"].values - df_lights["flux_err"].values, 1e-7, None))
    ...
    try:
        popt, pcov = curve_fit(linear_assym, x, y, p0=(x[max_ind], y[max_ind], 10, 300), sigma=s, method='trf', maxfev=3000,
